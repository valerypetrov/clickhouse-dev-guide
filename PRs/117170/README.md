...
### Round 6: the probe may not ask a cluster user for more than the write does

The follow-up review accepted that the probe no longer reads `system`, and found the narrower version of the same problem still there. `SHOW CREATE TABLE` and `DESC TABLE` check `SHOW COLUMNS` **on the table**, while a remote write's shard INSERT is authorized column by column. In `ContextAccess`, `res |= show_columns` is applied at the node holding the column flag, so `GRANT INSERT(metric_name, tags, time_series) ON db.t` gives `SHOW COLUMNS` on those columns and `SHOW TABLES` on the table, but never `SHOW COLUMNS` on the table. A cluster user holding exactly what the write sends could therefore do the write and be denied the check; the denial was collapsed into "unreachable" and the write refused with `ALL_CONNECTION_TRIES_FAILED`.

The fix (commit `cc0c50abc62`, patch 0012) is three lines: an `ACCESS_DENIED` from a replica's probe skips that replica rather than counting it unverified. This is only safe because of round 5. The write pins `insert_expected_table_engine` and `insert_expected_column_types`, so the shard's own insert refuses the target it resolves unless it is still a `TimeSeries` table of the type the wrapper declares; the preflight is an early, atomic refusal, not the only line of defence. A read never refused an unverified replica in the first place. Every other failure to answer is unchanged.

The test is the narrower sibling of round 5's. A third cluster entry names a user granted only `INSERT(metric_name, tags, time_series)` on the shard-local `TimeSeries` table and `INSERT` on that table's inner tables (looked up by the outer table's UUID, since the sink writes them by name on the caller's context), and nothing else. It asserts that both statements the probe makes are denied that user while an ordinary shard INSERT succeeds, that the remote write is acknowledged and every sample lands, and that over the same credentials a same-schema `MergeTree` target is still refused, by the shard's own insert, which names the engine it found and the one the INSERT expects. The wrapper declares only those three columns on purpose: `StorageDistributed::write` sends the wrapper's whole non-materialized sample block, so over a seven-column wrapper the shard INSERT would name the four metadata columns too and a three-column grant would not cover it. That is a real limit of the fix, not of the test: a column-granted cluster user works for a wrapper declaring the columns the write sends.

### Round 6b: what the full CI suite found

Until now only Fast test, Style check and the builds had run on most commits. The full suite finally ran on `cc0c50abc62` and turned up four failures, three of them the PR's own and one of them a regression I had introduced two rounds earlier. Fixed in `0fd7979367d`, patch 0013.

**A missing shard table was reported as an unreachable replica.** `test_a_missing_shard_table_is_refused_for_a_write_and_never_validated` asserts the refusal names "no table"; it got "(unreachable)". `DatabaseOnDisk::getCreateTableQueryImpl` answers a name it cannot produce a DDL for with `CANNOT_GET_CREATE_TABLE_QUERY`, not `UNKNOWN_TABLE`, so the round-5 probe's mapping missed it and everything fell through to the unreachable branch. The mapping now covers that code as well as `UNKNOWN_TABLE` and `UNKNOWN_DATABASE` (`DESC TABLE` uses the plain one). This is the second time the round-5 rewrite was bitten by an assumption about which code an error arrives with, the first being the `skip_unavailable_shards` mode; the lesson is that classifying by error code needs the code read from the throw site, not inferred from the operation.

**The stateless test deadlocked under randomized settings.** The last round's fix, moving the successful over-the-connection insert to a view-free table, addressed the wrong statement: the failure was on the very first `INSERT`, with `Lock timeout exceeded ... while pushing to view insert_expected_engine_mv_dist` after 60 s. The table carried two views, one writing a `Log` table directly and one writing that *same* `Log` table again through a `Distributed` table pointing back at this server, so the nested insert waited on the table lock the first view held for the whole statement. Fast test happened not to hit it; the flaky and targeted jobs, which randomize `max_threads` and friends, did. The nested hop only proved that a requirement a table consumed does not travel a further `Distributed` hop, which the view target of another type already proves without any hop, so both the hop table and its view are gone.

**Two assertions rested on premises that are not true.**
...
### Round 7: metric metadata a wrapper has no column to carry

The review's next Major, and a good one, because it falls straight out of the shape round 6 introduced. A `Distributed` table over per-shard `TimeSeries` tables need not declare all seven outer columns a `TimeSeries` table has: one declaring `metric_name`, `tags` and `time_series` carries samples perfectly well, and round 6's column-granted test deliberately uses that shape so the cluster user's grant can be column-scoped. But metric metadata lives in four columns of its own, and `makeMetricsMetadataBlock` read all four off the table the handler names, so a sender that put metadata in its `WriteRequest` got `NO_SUCH_COLUMN_IN_TABLE` from a table that looked fine for samples.

The reviewer offered two ways out: refuse the shape up front, or build the metadata columns from a real `TimeSeries` schema. The second cannot work. `StorageDistributed::write` sends exactly the columns the wrapper declares, so metadata the wrapper cannot name could never reach a shard that would store it; synthesizing the columns on the initiator would only move the failure to the `INSERT`. So the request is refused instead, before the shards are asked anything, with `INCOMPATIBLE_SCHEMA` (a 500, so the sender resends rather than dropping the batch) naming the missing column and what to declare. A `TimeSeries` table always has the four, so only a narrow wrapper is affected, and a request carrying no metadata is untouched.

The test sends one sample and its family's metadata in the single `WriteRequest` a sender puts both in (the idiom `test_prometheus_query_log.py` already uses), then asserts three things with the same request each time: through a wrapper of the full schema the metadata reaches the shards and reads back through `timeSeriesMetrics`, through the narrow wrapper the request is refused whole with neither samples nor metadata left on either shard, and the same samples without metadata still travel the narrow wrapper. The third is what pins the refusal to the metadata rather than to the wrapper.

Commit `898a600991e`, patch 0014. The branch owner had merged master again as `60d4bbb4efd` while this was being written, so the commit is rebased on top of that.

The branch owner later re-committed it as `ebe9405b49c` (identical tree, their committer identity), which is the parent of round 8.

### Round 8: the hidden CREATE TEMPORARY TABLE on every distributed read

The next Major, and a real one. The read rewrite builds `cluster(<c>, view(SELECT ... FROM timeSeriesSelector(<db>, '<table>', ...)))` and ships the query text to every shard, where the selector runs as the cluster entry's own user. `cluster()` and `view()` are both registered `allow_readonly`; `timeSeriesSelector` was not, and `ITableFunction::execute` asks for `CREATE TEMPORARY TABLE` for any table function that is not, before the function reads anything. So a cluster user granted exactly what the shard read needs - `SELECT` on the shard-local `TimeSeries` tables - failed every distributed PromQL read at runtime, and nothing on the initiator preflighted it: `checkPrometheusQueryDistributedRead` asked for that grant only when the cluster has a shard that is this server itself, and the docs said the query endpoints need it only there.

The reviewer offered either half of the fix; the right one is to stop demanding the grant at all. The selector only reads, and it asks for `SELECT` on the `TimeSeries` table itself in `StorageTimeSeriesSelector::getConfiguration` and again in `read()`, so `allow_readonly` gives away nothing - the precedent is `merge()` and `mergeTreeIndex()`, which read real tables and are readonly. With the selector readonly the local-shard preflight of that grant has nothing left to preflight and goes too: a shard that is this server itself is read in-process by the same readonly selector.

What does *not* change is the caller's own grant for naming a table function that is not readonly: `prometheusQuery`/`prometheusQueryRange` still need `CREATE TEMPORARY TABLE`, and `StoragePrometheusQuery::getConfiguration` still asks for it explicitly before `checkPrometheusQueryDistributedRead` runs, so a caller missing it is still denied before the shard probe can describe anything. That check is on the table-function path only (`getConfiguration` has one caller, `TableFunctionPrometheusQuery`), which is why the HTTP query endpoints now ask for it nowhere.

Two tests carry the change. In `test_write_distributed.py` the `two_shards_restricted` cluster user loses its `GRANT CREATE TEMPORARY TABLE ON *.*`, so the module's existing reads over that connection become regressions in themselves, and a new test asserts the premise (that user cannot create a temporary table), writes through the wrapper and reads every sample back over the same connection in SQL and over a new `/restricted_api/v1/query` handler. In `test_local_shard_distributed.py` the caller without the grant now reaches the probe instead of an access error; the table-function cases there still assert the denial, since `prometheusQuery`/`prometheusQueryRange` are still not readonly.

A third test was written and then removed, and the reason is worth keeping. It asserted the same thing in `test_access_distributed.py`, over the `two_shards_dist` cluster - and it was green before the fix as well. `Cluster::Address::isLocal` is `address.default_database.empty() && isLocalAddress(...)` (`src/Interpreters/Cluster.cpp:66`), and both replicas of that entry declare a `<default_database>`, so `getLocalShardCount()` is 0 and the branch the fix edited was never entered for that wrapper; the shards there also run as `default`, which holds the grant anyway. The property is only observable where a cluster entry has no `<default_database>` (the local-shard module) or names a user that lacks the grant (the restricted two-shard entry). An adversarial verification agent caught this after the first push; commit `a8d5119b423`, patch 0016, removes it and restores the ordering claim the docs rewrite had dropped.
...
All sixteen patches below were pushed to the PR branch `valerypetrov/ClickHouse:promql-over-distributed` (`a7a3a2501`, the round-2 follow-up `3c20def968ed6690c862b811f2b276e32fb6643e`, the comment-only `687057e62`, after the master merge `f49502b` the simplification `3000468a377` plus its assertion-message follow-up, and after the second merge `ad5ec290192` the round-3 witness `e5fe445b401` the fix that supersedes it, `4dbe1d113ec`, the round-4 local-replica fix `ed72d28924d`, its context-lifetime follow-up `d7617c6c3ac`, the round-5 fixes `a1d75e87b11`, after the third master merge `e7c4b2b82d0` the stateless-test fix `ef17cfdfb25`, the round-6 fix `cc0c50abc62`, the full-suite test fixes `0fd7979367d`, and the round-7 metadata fix, which the branch owner re-committed as `ebe9405b49c`, plus the round-8 readonly-selector fix `7c7ce6e0c8c` and its test follow-up `a8d5119b423`), so PR 117170 already carries them; the files are kept here as the record. The stand-down comment in section 2 still has to be posted on the PR by hand.
...
- `0012-Leave-a-replica-that-may-not-answer-the-shard-target.patch`: the round-6 fix, a replica that denies the probe left to its own delivery check, with the column-granted cluster-user test.
- `0013-Fix-the-four-tests-the-full-CI-suite-found-failing.patch`: the missing-table error code, the stateless test's view deadlock, and the two false premises.
- `0014-Refuse-remote-write-metadata-a-Distributed-table-has.patch`: the round-7 fix, refusing metric metadata a wrapper declares no column for, with the test over both wrapper shapes.
- `0015-Read-a-distributed-prometheus-query-without-CREATE-T.patch`: the round-8 fix, `timeSeriesSelector` registered readonly and the local-shard preflight of that grant dropped. Files: `src/TableFunctions/TableFunctionTimeSeries.cpp`, `src/Storages/TimeSeries/resolvePrometheusQueryTarget.cpp`, the docs, `tests/integration/test_prometheus_protocols/configs/prometheus_dist.xml` and the three distributed test modules.
- `0016-Drop-a-vacuous-test-and-restore-the-grant-s-ordering.patch`: the round-8 follow-up, removing the `test_access_distributed.py` case that was green either way and restoring the docs' ordering claim.
